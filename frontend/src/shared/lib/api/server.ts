...
export const serverApi = {
    ...
    /**
     * Get top banners for hero section
     * Note: This endpoint uses /api/top-banners (no /v1 prefix)
     */
    async getTopBanners(): Promise<TopBanner[]> {
        try {
            const url = `${BASE_URL}/top-banners`;
            const response = await fetch(url, {
                cache: 'no-store',
            ...
            if (!response.ok) {
                console.error(`[Server API] HTTP ${response.status} for top-banners`);
                return [];
            }
            ...
            console.error('[Server API] Error fetching top banners:', error);
            return [];
        }
    },
};
