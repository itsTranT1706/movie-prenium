...
export default function GenreCardGrid({ genres, showAll = false }: GenreCardGridProps & { showAll?: boolean }) {
    ...
    // Show 8 genres max on homepage (6 on lg, 8 on 2xl)
    const displayGenres = showAll ? allGenres : allGenres.slice(0, 8);
    ...
    return (
        <section className="relative pt-6 lg:pt-8 pb-4 lg:pb-5">
            <div className="w-full px-4 md:px-12 lg:px-16 2xl:px-12">
                ...
                <div className={`hidden lg:grid gap-3 ${showAll
                        ? 'grid-cols-3 xl:grid-cols-5 2xl:grid-cols-8'
                    ...
                    {displayGenres.map((genre, index) => (
                        <GenreCard
                            key={genre.id}
                            genre={genre}
                            className={!showAll && index >= 6 ? 'hidden 2xl:block' : ''}
                        />
                    ))}
                </div>
...
function GenreCard({ genre, mobile = false, className = '' }: { genre: Genre; mobile?: boolean; className?: string }) {
    return (
        <Link
            href={`/genre/${genre.id}`}
            className={`group ${mobile ? 'flex-shrink-0 w-[140px] snap-start' : 'w-auto'} ${className}`}
        >
            <div className={`
